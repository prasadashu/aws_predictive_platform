...
# Install npm modules
npm --prefix ./lambda/lambda-api-gateway-s3-integration/ install

# Zip modules into a package
cd ./lambda/lambda-api-gateway-s3-integration && zip -q -r function.zip node_modules/ package.json package-lock.json index.js && cd ../..
...
PLATFORM_LAMBDA_ARN=$(aws --endpoint-url=http://localhost:4566 lambda create-function \
    ...
    --zip-file fileb://./lambda/lambda-api-gateway-s3-integration/function.zip \
